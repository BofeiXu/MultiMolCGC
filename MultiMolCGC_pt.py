...
parser = argparse.ArgumentParser(description="Pretraining and Prediction Pipeline for MultiMolCGC")
parser.add_argument('--pretrain_csv', type=str, required=True,
                        help='Path to the CSV file used for pretraining.')
parser.add_argument('--pretrain_size', type=int, required=True,
                        help='Number of samples to use from the pretraining dataset.')
parser.add_argument('--data_csv', type=str, required=True,
                        help='Path to the dataset CSV file for prediction.')
...
def load_graph_dataset(csv_path, start=0, finish=0, pct=1, shuffle=True, pt=True):
...
class MTLModelCGC_graph_protein(nn.Module):
    ...
    def forward(self, data, ft=False):
        ...
        for task, experts in self.task_specific_experts.items():
            ...
            # pretraining - learn fused rep

            if not ft:
                out = self.task_heads[task](fused_rep)
            else:
                out = self.ft_heads[task](fused_rep)
...
if __name__ == '__main__':
    ...
    batch_size = 256
    pretrain_wd = 0
    pretrain_multistep_lr = [7*i for i in range(1, 993)]

    hidden_dim = 256

    vina_data_list = load_graph_dataset(
        csv_path=args.pretrain_csv,
        start=0,
        finish=args.pretrain_size,
        pct=None,
        shuffle=False
    )
    sample_data = vina_data_list[0]
    ...
    vina_train_dataset, vina_test_dataset = train_test_split(
            vina_data_list, test_size=0.05, random_state=42
        )
    vina_train_loader = GeoDataLoader(vina_train_dataset, batch_size=batch_size, shuffle=True)
    vina_test_loader = GeoDataLoader(vina_test_dataset, batch_size=batch_size, shuffle=False)

    model = MTLModelCGC_graph_protein(
        input_dim=input_dim, edge_dim=edge_dim,
        shared_hidden_dim=hidden_dim, individual_hidden_dim=hidden_dim,
        num_experts=2, num_heads=1, dp=0.2,
        protein_embed_dim=embed_dim[pro_embed_model]
    ).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=pretrain_lr, weight_decay=pretrain_wd)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=pretrain_multistep_lr, gamma=0.8)
    ...
    model.train()
    with tqdm(range(pretrain_epochs)) as epochs:
        for epoch in epochs:
            model.train()
            total_train_loss = 0
            for batch_data in vina_train_loader:
            ...
            ave_train_loss = total_train_loss / len(vina_train_loader)
            pt_train_loss_curve.append(np.sqrt(ave_train_loss))

            # EVAL
            model.eval()
            total_test_loss = 0
            with torch.no_grad():
                for batch_data in vina_test_loader:
                    batch_data = batch_data.to(device)
                    pred = model(batch_data)
                    loss = masked_loss(pred, batch_data.y)
                    total_test_loss += loss.item()
            ave_test_loss = total_test_loss / len(vina_test_loader)
            pt_test_loss_curve.append(np.sqrt(ave_test_loss))
    ...
    for i, task_name in enumerate(TASKS):
        ...
        # axs = axs.ravel()
        # ax=axs[0]
        plt.plot(range(len(pt_train_loss_curve)), pt_train_loss_curve, c='red')
        plt.plot(range(len(pt_train_loss_curve)), pt_test_loss_curve, c='blue')
        plt.savefig(f'epoch_plots/pt_seed_{42}_curve_CGC.png')

    # SAVE PRETRAINED MODEL
    torch.save(model.state_dict(), "mol_pretrained.pt")
    ...
    full_data_list = load_graph_dataset(csv_path, start=0, finish=100 , pct=1, shuffle=True, pt=False)
    ...
    for cluster in range(num_clusters):
        ...
        individual_hidden_dim=hidden_dim,
        num_experts=2,
        num_heads=1,
        dp=0.2,
        protein_embed_dim=embed_dim[pro_embed_model]).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=wd)
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=multistep_lr, gamma=0.8)

        model.load_state_dict(torch.load("mol_pretrained.pt"))

        if freeze:
            for name, param in model.named_parameters():
                if not name.startswith("ft_heads"):
        ...
        with tqdm(range(train_epoch)) as epochs:
            for epoch in epochs:
                ...
                for batch_data in train_loader:
                    batch_data = batch_data.to(device)
                    pred, _ = model(batch_data, ft=True)
                    #print(pred.shape, batch_data.y.shape)
                    loss = masked_loss(pred, batch_data.y)
                    # print(loss)
                    optimizer.zero_grad()
        ...
        with torch.no_grad():
            for batch_data in test_loader:
                batch_data = batch_data.to(device)
                pred, task_reps = model(batch_data, ft=True)
                y_true_all.append(batch_data.y.cpu().numpy())
                y_pred_all.append(pred.cpu().numpy())
        ...
        task_reps_sars_all = np.concatenate(task_reps_sars, axis=0)
        task_reps_mers_all = np.concatenate(task_reps_mers, axis=0)
        ...
        plt.savefig(f'epoch_plots/PT_Cluster_{cluster}_curve_CGC.png')
